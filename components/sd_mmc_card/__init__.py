...
async def to_code(config):
    ...
    if CORE.is_esp32:
        # Declare the IDF components this component depends on.
        # ESPHome's esp-idf build system will add these to the REQUIRES list
        # of its generated main IDF component, which makes their include
        # directories available to the compiler automatically — no manual
        # path injection needed.
        #
        # IDF 5.x split the sdmmc driver into esp_driver_sdmmc; we list both
        # so the component works with IDF 4 and 5.
        for idf_component in [
            "fatfs",              # esp_vfs_fat.h, ff.h
            "sdmmc",              # sdmmc_cmd.h, driver/sdmmc_defs.h
            "esp_driver_sdmmc",   # driver/sdmmc_host.h  (IDF 5+)
            "driver",             # driver/sdmmc_host.h  (IDF 4 compat)
            "vfs",                # pulled in transitively; explicit for safety
        ]:
            cg.add_idf_component_dependency(idf_component)